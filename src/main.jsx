...
const seed = {
  ...
  suppliers: [{ id: 'supplier-1', name: 'NSDL', currency: 'USD', current_balance: 0, revenue_share_percent: 100, cogs_percent: 22, buffer_percent: 10 }],
  adAccounts: [{ id: 'ad-1', name: 'Ad1', platform: 'Meta', currency: 'USD', current_balance: 0, daily_spend: 0, roas_3d: 0 }],
  opexItems: [
    { id: 'opex-1', name: 'Payroll', amount: 0 },
    { id: 'opex-2', name: 'Subscriptions', amount: 0 },
    { id: 'opex-3', name: 'Misc', amount: 0 }
  ],
  settings: { display_currency: 'USD', payout_delay_business_days: 5, scale_percent: 20, roas_threshold: 1.8, owner_draw_target: 500, owner_draw_currency: 'USD' }
};
...
function App() {
  ...
  function normalizeSettings(row) {
    return {
      ...
      payout_delay_business_days: row.payout_delay_business_days ?? 5,
    ...
    <div className="app-shell">
      ...
      {activeTab === 'inputs' && dashboard && (
        <main className="settings-grid">
          ...
          <SettingsPanel title="Supplier Planning" action={<AddButton onClick={() => insertRow('supplier_plans', { id: uid('supplier'), name: 'New Supplier', currency: settings.display_currency, current_balance: 0, revenue_share_percent: 100, cogs_percent: 22, buffer_percent: 10 }, setSuppliers)} />}>
          ...
          <SettingsPanel title="Ad Planning" action={<AddButton onClick={() => insertRow('ad_accounts', { id: uid('ad'), name: 'New Ad Account', platform: 'Meta', currency: settings.display_currency, current_balance: 0, daily_spend: 0, roas_3d: 0 }, setAdAccounts)} />}>
...
function EditableSupplier({ supplier, update, remove }) { return <div className="edit-card"><TextField label="Supplier name" value={supplier.name} onChange={v => update({ name: v })} /><Field label="Currency" type="select" value={supplier.currency} onChange={v => update({ currency: v })} options={CURRENCIES} /><Field label="Current supplier balance" value={supplier.current_balance} onChange={v => update({ current_balance: v })} /><Field label="Revenue share %" value={supplier.revenue_share_percent ?? 100} onChange={v => update({ revenue_share_percent: v })} /><Field label="COGS %" value={supplier.cogs_percent} onChange={v => update({ cogs_percent: v })} /><Field label="Buffer %" value={supplier.buffer_percent} onChange={v => update({ buffer_percent: v })} /><button className="danger" onClick={remove}><Trash2 size={15} /> Delete</button></div>; }
function EditableAdAccount({ account, update, remove }) { return <div className="edit-card"><TextField label="Name" value={account.name} onChange={v => update({ name: v })} /><Field label="Currency" type="select" value={account.currency} onChange={v => update({ currency: v })} options={CURRENCIES} /><Field label="Current balance" value={account.current_balance} onChange={v => update({ current_balance: v })} /><Field label="Daily spend" value={account.daily_spend} onChange={v => update({ daily_spend: v })} /><Field label="3D ROAS" value={account.roas_3d} onChange={v => update({ roas_3d: v })} /><button className="danger" onClick={remove}><Trash2 size={15} /> Delete</button></div>; }
